...
MKLDNNPlugin::ConvertMatMulToFC::ConvertMatMulToFC() {
    ...
    ngraph::matcher_pass_callback callback = [this](ngraph::pattern::Matcher& m) {
        ...
        auto shape_a = input_a.get_partial_shape();
        auto shape_b = input_b.get_partial_shape();
        ...
        auto get_aligned_shapes = [shape_a, shape_b, rank_a, rank_b, &matmul]() -> std::tuple<bool, ngraph::PartialShape, ngraph::PartialShape> {
            std::vector<ngraph::Dimension> shape_a_aligned(shape_a), shape_b_aligned(shape_b);
            ...
            for (size_t i = 0; i < max_size - 2; ++i) {
                ...
                if (a_dim.is_dynamic() || b_dim.is_dynamic()) {
                    if (a_dim == 1)
                        shape_a_aligned[i] = shape_b_aligned[i] = b_dim;
                    else if (b_dim == 1)
                        shape_a_aligned[i] = shape_b_aligned[i] = a_dim;
                    else
                        return {false, shape_a_aligned, shape_b_aligned};
                    continue;
                }
        ...
        auto create_transpose = [this](ngraph::Output<ngraph::Node> node, const std::string& transpose_name) -> std::pair<bool, std::shared_ptr<ngraph::Node>> {
            auto rank = node.get_partial_shape().rank();
            if (rank.is_dynamic())
                return {false, node.get_node_shared_ptr()};
        ...
        if ((std::dynamic_pointer_cast<ngraph::opset1::Constant>(fc_input_b.get_node_shared_ptr()) ||
            ...
            if (aligned_a_rank.is_dynamic() || aligned_b_rank.is_dynamic())
                return false;
            ...
            ngraph::Dimension K = *(shape_a_aligned.end() - 1);
            std::vector<ngraph::Dimension> B(shape_a_aligned.begin(), shape_a_aligned.end() - 2);

            // Weights normalization
            if (!matmul->get_transpose_b()) {
                std::tie(success, fc_input_b) = create_transpose(fc_input_b, matmul->get_friendly_name() + "/transpose_b");
                if (!success)
                    return false;
            ...
            if (rank_b != 2) {
                if (K.is_dynamic())
                    return false;
                auto reshape_shape =
                        ngraph::opset1::Constant::create<int64_t>(ngraph::element::i64, ngraph::Shape{2}, {-1ll, static_cast<int64_t>(K.get_length())});
            ...
            // Input normalization
            if (matmul->get_transpose_a() && rank_a != 1) {
                std::tie(success, fc_input_a) = create_transpose(fc_input_a, matmul->get_friendly_name() + "/transpose_a");
                if (!success)
                    return false;
